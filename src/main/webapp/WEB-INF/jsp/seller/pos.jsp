                ...
                <div class="container-fluid px-4 py-4">
                    ...
                    <div class="fm-page-header mb-4 border-bottom pb-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <div class="fm-caption fw-bold text-primary mb-1 text-uppercase ls-wide">Counter
                                    Operations</div>
                    ...
                    <c:if test="${not empty sessionScope.sellerPosSuccessMessage}">
                        <div class="alert alert-success fm-surface border-0 shadow-sm mb-4 p-3 anim-fade-in shadow-sm">
                            ...
                            <c:out value="${sessionScope.sellerPosSuccessMessage}" />
                        </div>
                    ...
                    <c:if test="${not empty sessionScope.sellerPosErrorMessage}">
                        <div class="alert alert-danger fm-surface border-0 shadow-sm mb-4 p-3 anim-fade-in shadow-sm">
                            ...
                            <c:out value="${sessionScope.sellerPosErrorMessage}" />
                        </div>
                        ...
                        <div class="fm-pos-inventory">
                            <div class="fm-surface p-3 mb-4 shadow-sm border-0 d-flex gap-3">
                                <div class="flex-grow-1">
                                    <div class="input-group">
                                        ...
                                        <input type="text" id="posSearch" class="fm-form-control border-start-0"
                                            placeholder="Scan barcode or type SKU name..." onkeyup="filterPOS()" />
                                    </div>
                                </div>
                            ...
                            <div class="fm-catalog-grid" id="posItems">
                                <c:forEach items="${products}" var="p">
                                    <div class="fm-pos-item-card ${availableMap[p.id] <= 0 ? 'disabled' : ''}"
                                        data-name="${fn:toUpperCase(p.name)}">
                                        <div>
                                            <div class="d-flex justify-content-between align-items-start mb-2">
                                                <span
                                                    class="badge bg-slate-100 text-slate-500 font-monospace small">#${p.id}</span>
                                                <c:choose>
                                            ...
                                            <div class="text-primary fw-bold font-monospace fs-5">?
                                                <fmt:formatNumber value="${p.sellPrice}" groupingUsed="true" />
                                            </div>
                                        </div>

                                        <div class="mt-3">
                                            <div class="d-flex justify-content-between small text-muted mb-2">
                                                <span>ATP: <b>${availableMap[p.id]}</b></span>
                                                <span>Exp: <b>${nearestExpiryMap[p.id] != null ? nearestExpiryMap[p.id]
                                                        : 'N/A'}</b></span>
                                            </div>
                                            <form method="post" action="${pageContext.request.contextPath}/seller/pos">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    <input class="form-control text-center" name="quantity"
                                                        type="number" min="1" value="1" style="max-width: 60px;" />
                                                    <button class="btn btn-primary fw-bold flex-grow-1" type="submit"
                                                        ${availableMap[p.id] <=0 ? 'disabled' : '' }>
                        ...
                        <div class="fm-pos-cart-panel shadow-lg border-0">
                            ...
                            <div class="pos-cart-items">
                                <c:if test="${empty lines}">
                                    <div
                                        class="h-100 d-flex flex-column align-items-center justify-content-center text-center opacity-25">
                                    ...
                                    <div class="vstack gap-3">
                                        <c:forEach items="${lines}" var="l">
                                            <div class="p-3 border rounded-4 bg-light position-relative">
                                                <form method="post"
                                                    action="${pageContext.request.contextPath}/seller/pos">
                                                    <input type="hidden" name="csrf_token"
                                                        value="${sessionScope.CSRF_TOKEN}" />
                                                ...
                                                <div class="d-flex justify-content-between align-items-start mb-1">
                                                    ...
                                                    <div
                                                        class="bg-indigo-100 text-indigo-700 px-2 py-1 rounded small fw-bold">
                                                        ?
                                                        <fmt:formatNumber value="${l.lineTotal}" groupingUsed="true" />
                                                    </div>
                                                </div>
                                                <div
                                                    class="d-flex justify-content-between align-items-center small text-muted">
                                                    <span>${l.quantity} × ?
                                                        <fmt:formatNumber value="${l.product.sellPrice}"
                                                            groupingUsed="true" />
                                                    </span>
                                                    <c:choose>
                                                        <c:when test="${cartShortageMap[l.product.id] > 0}">
                                                            <span class="text-danger fw-bold"><i
                                                                    class="bi bi-exclamation-triangle"></i> ATP short:
                                                                ${cartShortageMap[l.product.id]}</span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span class="text-success fw-bold"><i
                                                                    class="bi bi-check-circle"></i> ATP OK</span>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                            </div>
                            ...
                            <div class="pos-cart-footer">
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <span class="fm-h2 mb-0 text-primary">?
                                ...
                                <c:if test="${not empty lines}">
                                    <form method="post" action="${pageContext.request.contextPath}/seller/pos">
                                        ...
                                        <div class="mb-3">
                                            <label class="fm-caption fw-bold d-block mb-2 text-muted">Settlement
                                                Method</label>
                                        ...
                                        <button class="fm-btn fm-btn-primary w-100 py-3 fw-bold shadow-lg" type="submit"
                                            ${cartHasShortage ? 'disabled' : '' }>
                                    ...
                                    <form method="post" action="${pageContext.request.contextPath}/seller/pos"
                                        ...
                                        <button
                                            class="btn btn-link text-danger w-100 border-0 text-decoration-none fw-bold small"
                                            type="submit">
