            ...
            <c:if test="${detailView != null}">
                ...
                <c:if test="${order.status == 'PENDING' || order.status == 'PROCESSING' || order.status == 'SHIPPING'}">
                    <div class="alert alert-light border">
                        Đơn ở trạng thái chưa hoàn tất đang dùng cơ chế <strong>reserve theo FEFO</strong>.
                        Khi completed, reservation sẽ được convert thành actual allocation và trừ kho thật.
                    </div>
                </c:if>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            ...
                            <c:if test="${order.status == 'PENDING'}">
                                <form method="post"
                                    action="${pageContext.request.contextPath}/staff/orders/update-status"
                                    class="d-inline">
                                ...
                                <form method="post" action="${pageContext.request.contextPath}/staff/orders/complete"
                                    class="d-inline">
                                    <input type="hidden" name="csrf_token" value="${sessionScope.CSRF_TOKEN}" />
                                    <input type="hidden" name="id" value="${order.id}" />
                                    <button class="btn btn-success" type="submit" ${!detailView.allFulfillable
                                        ? 'disabled="disabled"' : '' }>
                            ...
                            <div class="border rounded p-3 mb-3">
                                ...
                                <c:if test="${line.hasActualAllocations}">
                                    <div class="alert alert-light border small mb-3">
                                        Đây là lịch sử xuất kho thực tế của order item #${line.orderItemId}. Tổng đã cấp
                                        phát: <strong>${line.actualAllocatedQty}</strong>.
                                    </div>
                                    <table class="table table-sm align-middle mb-0">
                                        ...
                                        <tbody>
                                            <c:forEach items="${line.actualAllocations}" var="alloc">
                                                <tr>
                                                    <td>
                                                        <c:out value="${alloc.productLot.id}" />
                                                    </td>
                                                    <td>
                                                        <c:out value="${alloc.productLot.importDate}" />
                                                    </td>
                                                    <td>
                                                        <c:out value="${alloc.productLot.expiryDate}" />
                                                    </td>
                                                    <td><strong>
                                                            <c:out value="${alloc.allocatedQty}" />
                                                        </strong></td>
                                                    <td>
                                                        <c:out value="${alloc.createdAt}" />
                                                    </td>
                                                </tr>
                                            </c:forEach>
                                        </tbody>
                                ...
                                <c:if test="${not line.hasActualAllocations and not empty line.plan.allocations}">
                                    <div class="small text-muted mb-2">Đây là preview FEFO theo ATP hiện tại, chưa phải
                                        trace thực tế.</div>
                                    <table class="table table-sm align-middle mb-0">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <c:forEach items="${line.plan.allocations}" var="alloc">
                                                <tr class="${alloc.nearExpiry ? 'table-warning' : ''}">
                                                    <td>
                                                        <c:out value="${alloc.lotId}" />
                                                    </td>
                                                    <td>
                                                        <c:out value="${alloc.importDate}" />
                                                    </td>
                                                    <td>
                                                        <c:out value="${alloc.expiryDate}" />
                                                    </td>
                                                    <td>
                                                        <c:out value="${alloc.qtyLeftBefore}" />
                                                    </td>
                                                    <td><strong>
                                                            <c:out value="${alloc.allocatedQty}" />
                                                        </strong></td>
