...
<div class="fm-page-header">
    ...
    <div class="d-flex gap-2 flex-wrap">
        <a class="btn btn-outline-secondary" href="${pageContext.request.contextPath}/catalog">Về Catalog</a>
        <a class="btn btn-outline-secondary" href="${pageContext.request.contextPath}/customer/dashboard">Customer Dashboard</a>
...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    ...
    <div>
        <c:choose>
            <c:when test="${sessionScope.authUser != null && sessionScope.authUser.tier.toString() eq 'PRO' && sessionScope.authUser.proActive}">
                <span class="badge text-bg-success fs-6">
                    PRO đến <c:out value="${sessionScope.authUser.expiredDate}"/>
                </span>
            </c:when>
            <c:when test="${sessionScope.authUser != null && sessionScope.authUser.tier.toString() eq 'PRO'}">
                <span class="badge text-bg-warning fs-6">PRO đã hết hạn</span>
            </c:when>
            <c:otherwise>
            ...
            <c:choose>
                <c:when test="${sessionScope.authUser != null}">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-7">
                            <c:choose>
                                <c:when test="${sessionScope.authUser.proActive}">
                                    <span class="text-success fw-semibold">
                                        Đang hoạt động
                                        <c:if test="${sessionScope.authUser.remainingProDays >= 0}">
                                            (<c:out value="${sessionScope.authUser.remainingProDays}"/> ngày còn lại)
                                        </c:if>
                                    </span>
                                </c:when>
                                <c:when test="${sessionScope.authUser.tier.toString() eq 'PRO'}">
                                    <span class="text-danger fw-semibold">Đã hết hạn</span>
                                </c:when>
                                <c:otherwise>
    ...
    <div class="col-lg-7">
        <div class="fm-surface padded h-100">
            ...
            <div class="alert alert-info mt-3 mb-0">
                Đây là màn hình fake payment để phục vụ demo chức năng subscription / upgrade / payment history.
            </div>
        </div>
    </div>
</div>
                            ...
                            ">
                                <div class="card-body d-flex flex-column">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <h5>PRO <c:out value="${plan.key}"/> ngày</h5>
                                        </c:otherwise>
                                    </c:choose>
                                    ...
                                    <div class="text-muted small mb-3">
                                        Gia hạn sẽ được cộng dồn nếu tài khoản PRO hiện vẫn còn hiệu lực.
                                    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        ...
        <span class="text-muted small">
            <c:choose>
                ...
                <c:otherwise><c:out value="${paymentHistory.size()}"/> giao dịch</c:otherwise>
            </c:choose>
        </span>
    </div>
                ...
                <table class="table fm-table table-striped align-middle">
                    ...
                    <c:forEach items="${paymentHistory}" var="p">
                        <tr>
                            ...
                            <td>
                                <fmt:formatNumber value="${p.amount}" type="number" maxFractionDigits="0"/> đ
                            </td>
